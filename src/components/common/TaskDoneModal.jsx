...
function TaskDoneModal({ isOpen, issue, onConfirm, onCancel }) {
  ...
  // ── Helpers ────────────────────────────────────────────────────────────────
  function dispatchActivity(description, type = ACTIVITY_TYPES.STATUS_CHANGE) {
  ...
  function handleConfirm() {
    ...
    // 2. Stock check (only if equipment was used and warning not already shown)
    ...
    // 3. Dispatch all actions
    const now = new Date().toISOString();

    // Move issue to Done
    dispatch({
      type: ACTIONS.MOVE_ISSUE,
      payload: { issueId: issue.id, newStatus: 'Done' },
    });

    // Çözüm notunu issue üzerinde sakla (RequestCard'da gösterim için)
    dispatch({
      type: ACTIONS.UPDATE_ISSUE,
      payload: { id: issue.id, resolutionNote: resolutionNote.trim() },
    });
    ...
    dispatchActivity(
      `Durum "${issue.status}" → "Done" olarak değiştirildi`,
      ACTIVITY_TYPES.STATUS_CHANGE
    );

    // Update resolvedAt
    dispatch({
      type: ACTIONS.UPDATE_REQUEST_DATES,
      payload: {
        issueId: issue.id,
        resolvedAt: now,
    ...
    if (usedEquipment && selectedEquipmentId) {
      ...
      dispatch({
        ...
        payload: {
          inventoryId: selectedEquipmentId,
          quantityChange: -requestedQty,
        },
      });

      dispatchActivity(
        `Stok düşümü: ${selectedEquipment?.name || selectedEquipmentId} — ${requestedQty} ${selectedEquipment?.unit || 'adet'}`,
  ...
  function handleStockWarningContinue() {
    setShowStockWarning(false);
    // Re-run confirm without triggering stock check again
    const now = new Date().toISOString();
